...
当前主线为：`素材进入 → 结构拆解 → 排版模式 → 业务检索 → 人工反馈`。
本阶段只收口现有16个候选排版模式并建立人工确认流程；页面入口为
`http://127.0.0.1:3100/patterns`。系统只提供保留、合并、拒绝建议，任何候选都不会自动删除或自动写入 `verified`。

正式模式必须具有至少3个公司案例、明确代表案例，并由人工确认名称、适用/不适用场景、必需/可选模块，最后由设计负责人确认。完整边界见 [CURRENT_STAGE.md](CURRENT_STAGE.md)。

暂停但不删除代码：视觉模型批量分析、Calibration/Holdout、奶瓶导入、模型微调、候选设计审核智能体、图片生成和非主线页面优化。
