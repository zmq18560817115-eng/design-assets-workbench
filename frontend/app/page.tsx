...
export default function Dashboard() {
  ...
  const tasks = useMemo(() => [
    ...
    ["待人工审核", cases.filter((item) => item.trust_status === "ai_unverified").length],
    ["已确认蓝图", cases.filter((item) => item.trust_status === "verified" || item.trust_status === "company_recommended").length],
    ["待审核排版模式", 0], ["最近业务检索", 0],
  ] as const, [cases]);
  const next = tasks[1][1] ? "/assets?tab=review" : tasks[3][1] ? "/assets?tab=review" : tasks[2][1] ? "/assets?tab=library" : "/patterns";
