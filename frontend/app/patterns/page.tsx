...
type Candidate = {
  ...
  evidence_preview: { own_case_count: number; merged_candidate_count: number; evidence_count: number;
    source_candidate_ids: string[]; case_ids: number[]; deduplicated_count: number;
    case_deduplicated_count: number;
    excluded_evidence: { id: number; reason: string }[]; errors: string[] };
};
      ...
      {visible.map(candidate => {
        ...
        return <Card key={candidate.candidate_id} className="h-full">
          ...
          <div className="mt-4 space-y-2">
            ...
            <div className="rounded-xl bg-gray-50 p-3 text-xs"><div className="flex flex-wrap gap-4"><span>自身案例：<strong>{candidate.evidence_preview.own_case_count}</strong></span><span>合并候选：<strong>{candidate.evidence_preview.merged_candidate_count}个</strong></span><span>聚合后案例：<strong>{candidate.evidence_preview.evidence_count}</strong></span></div><details className="mt-2"><summary className="cursor-pointer font-medium">查看证据聚合预览</summary><div className="mt-2 space-y-1 text-gray-500"><p>来源候选：{candidate.evidence_preview.source_candidate_ids.join("、")}</p><p>案例ID：{candidate.evidence_preview.case_ids.join("、")}</p><p>标注去重：{candidate.evidence_preview.deduplicated_count}；案例去重：{candidate.evidence_preview.case_deduplicated_count}</p><p>排除证据：{candidate.evidence_preview.excluded_evidence.length ? candidate.evidence_preview.excluded_evidence.map(item => `${item.id}:${item.reason}`).join("、") : "无"}</p>{candidate.evidence_preview.errors.length > 0 && <p className="text-rose-700">聚合错误：{candidate.evidence_preview.errors.join("；")}</p>}</div></details></div>
          </div>
