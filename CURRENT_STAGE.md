...
消毒柜彩框方法已抽象为全品类标注合同和通用接口。系统可以对吸奶器、奶瓶、
恒温杯、羊脂膏等已有 Case 使用相同的结构反推流程，但“可运行”不等于“该品类
已经学好”：每个品类的公司标准分别计数，至少 30 张人工 verified 并保留 holdout
后才进入正式评估。跨品类公司成品只借用结构，外部素材只在模仿模式中使用，
二者都不能冒充目标品类的公司业务标准。
